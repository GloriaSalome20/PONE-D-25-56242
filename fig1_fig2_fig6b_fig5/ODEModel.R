...
runODE = function(tMax, tIncrement, initState, theta, method){
  ...
  IVM_traj <- data.frame(dede(y = initState, times = times, 
                              parms = theta, func = IVM_ode, method = method))
  #browser()
...
calculateInitialState = function(theta){
  ...
  ##What about humans in equilbrium?/
...
#browser()
  ...
  feedingCycleImpact= impactFeedingCycleParameters(time,beta,tau1,tau2,e_ov,time_ATSB_on,ATSBcov,
                                                   time_SSP_on,SSPcov,fSSP,fATSB,muV,Q0,aOBT,OBTcov,
                                                   time_OBT_on,time_ITN_on,ITNcov,time_IRS_on,IRScov,
                                                   HOUcov,time_HOU_on,rITN,sITN,rIRS,rHOU,sIRS,sHOU, 
                                                   phiB, phiI,dHOU,dIRS,time_SPR_on,SPRcov,time_PPM_on,
                                                   PPMcov,rSPR,rPPM,sSPR,sPPM,c0,time_ECS_on,ECScov,
                                                   time_ECT_on,ECTcov,rECT,sECS,sECT,time_OVI_on,OVIcov,
                                                   fOVI,eOVI)
  ...
  #HBI<- computeHBI(wCom_human,wCom)
  #hbr <- computeHumanBitingRate(f_theta,HBI_com)
 ...
 #Introducing males
  ...
  return(list(c(dEL,dEL_LAR,dEL_BIO,dEL_LAR_BIO, dLL,dLL_LAR,dLL_BIO,dLL_LAR_BIO, dPL, 
                dSV, dEV, dIV,dSH,dIH,dMm), EIR=EIR, VC = VC, R0 = R0,Mdensity=Mdensity,a_theta=a_theta))
