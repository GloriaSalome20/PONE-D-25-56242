...
# ggplot(IVM_traj_0,aes(x=time,y=EIR))+
#   geom_line()
...
# par(font.axis=2, font.lab=2, cex=1.4) # bold the axis values and labels
# 
# plot(new_eir[,1], type="n", ylim=c(0,max(new_eir)), 
#      ylab = "Entomological inoculation rate", 
#      xlab = "simulation days", main = "")
# 
# EIRlwd <- 2
# 
# # looping the coverages into one plot
# for(i in 1:6){
#   lines(new_eir[,i], col=i, lwd=EIRlwd)
# }
# 
# legend("right", col=1:6, lwd=EIRlwd,
#        legend = paste0(INITIAL_LAR_COVERAGE*100, "%"))
# 
...
larv60_plot<-ggplot(data = new_eir) +
  ...
  labs(x = "Time (Days)", y = "Entomological Inoculation Rate", 
       color = "ITN Coverages \n for 60% Larv", linetype = "ITN Coverages \n for 60% Larv") +
  scale_color_manual(
...
ggsave("larv60_plot.png", plot = larv60_plot, width = 10, height = 6, units = "in", dpi = 300)
ggsave("Figure2_PLOSONE_lineart.tiff", p, device = "tiff",
       width = 7, height = 5, units = "in", dpi = 900, compression = "lzw")
