...
eir_plot<-ggplot() +
  ...
  ylim(-NA, NA) +
  geom_line(data=IVM_traj_0, aes(x=time, y=EIR,colour="Before Larv"),size=1.75) + 
  geom_line(data=IVM_traj_2, aes(x=time, y=EIR,colour="After Larv"),size=1.75)+
  labs(x = "Time (Days)", y = "Entomological Inoculation Rate", color = "Scenario")+
  theme_classic()+
  theme(
...
ggsave("eir_plot.png", plot = eir_plot, width = 10, height = 6, units = "in", dpi = 300)

# # Calculate the total number of infected cases before and after intervention
# total_cases_before <- sum(IVM_traj_0$EIR)
