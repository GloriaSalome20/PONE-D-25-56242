...
plotTrajectoryimmature = function(IVM_traj){
  ...
  ggplot(IVM_traj, aes(x = time, y = IVM_traj, color = State)) +
    ...
    geom_line(aes(y = EL, col = "Early Instar"), size = 1.75) +
    geom_line(aes(y = LL, col = "Late Instar"), size = 1.75) +
    geom_line(aes(y = PL, col = "Pupae"), size = 1.75) +
    ylim(0, NA) +
    theme_classic() +
    labs(x = "Time (days)", y = "Number of Immature mosquitoes")+
